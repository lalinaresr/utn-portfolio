...
if (count($_POST) > 0) {
    ...
    mysqli_query($connection, sprintf("INSERT INTO projects VALUES ('%d', '%d', '%s', '%s', '%s', '%s', '%s', '%s', NULL, NULL) ON DUPLICATE KEY UPDATE type_id = '%d', name = '%s', slug = '%s', description = '%s', link = '%s', image = '%s', updated_at = '%d'", $id, $type, $name, strtolower($name), $description, $link, $image, date('Y-m-d H:i:s'), $type, $name, strtolower($name), $description, $link, $image, date('Y-m-d H:i:s')));
