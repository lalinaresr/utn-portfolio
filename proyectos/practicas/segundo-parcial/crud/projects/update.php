...
if (count($_POST) > 0) {
    ...
    if (mysqli_num_rows($response) == 0) {
        mysqli_query($connection, sprintf("UPDATE projects SET type_id = '%d', name =  '%s', slug = '%s', description = '%s', link = '%s', image = '%s', updated_at  ='%s' WHERE id = '%d'", $type, $name, strtolower($name), $description, $link, $image, date('Y-m-d H:i:s'), $id));
